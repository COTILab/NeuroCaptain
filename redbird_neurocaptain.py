...
# Solver selection: 'mmc' or 'redbird'
SOLVER = 'redbird'  # Change to 'mmc' to use Monte Carlo

# Shared settings
SD_MAX_DIST = 60.0    # Max source-detector distance
MIN_DEPTH = 2.0       # Minimum projection depth into mesh
SMOOTH_ITERATIONS = 5 # Smoothing passes for visualization

# MMC-specific settings
NPHOTON = 1000        # For MMC: photon count
SKIP_SOURCES = []     # MMC only: skip problematic sources
SKIP_DETECTORS = [17] # MMC only: skip problematic detectors

# Redbird-specific settings
REDBIRD_OMEGA = 2 * np.pi * 0  # Angular frequency (0 for CW, 70e6 for 70MHz FD)

# Optical properties (same format works for both solvers)
OPTICAL_PROPERTIES = {
...
def project_into_mesh(surface_pos, normal_in, nodes, elems, tree=None, centroids=None):
    ...
    # Fallback: move toward nearest element
...
def _print_timing_summary(fwd_times, adj_times, t_sim_total, ns, nd, nphoton=None):
    """Print a table of per-simulation timing analytics."""
    print("\n  " + "-" * 56)
    print("  TIMING SUMMARY")
    print("  " + "-" * 56)
    if fwd_times:
        print(f"  Forward  ({len(fwd_times):>2}/{ns}):  "
              f"{sum(fwd_times):7.1f}s total | "
              f"{np.mean(fwd_times):.2f}s avg | "
              f"{np.min(fwd_times):.2f}–{np.max(fwd_times):.2f}s range | "
              f"std={np.std(fwd_times):.2f}s")
    if adj_times:
        print(f"  Adjoint  ({len(adj_times):>2}/{nd}):  "
              f"{sum(adj_times):7.1f}s total | "
              f"{np.mean(adj_times):.2f}s avg | "
              f"{np.min(adj_times):.2f}–{np.max(adj_times):.2f}s range | "
              f"std={np.std(adj_times):.2f}s")
    all_times = fwd_times + adj_times
    if all_times:
        throughput = len(all_times) / max(t_sim_total, 1e-6)
        print(f"  Throughput: {throughput:.3f} sims/s  ({throughput * 60:.1f} sims/min)")
    if nphoton is not None and fwd_times:
        pps = nphoton / np.mean(fwd_times)
        print(f"  MMC photons/sec (avg per sim): {pps:>12,.0f}")
    print(f"  Simulation wall time: {t_sim_total:.1f}s  ({t_sim_total / 60:.2f} min)")
    print("  " + "-" * 56)
...
def prepare_redbird_mesh(nodes, elems, tissue_labels):
    ...
    Redbird uses 1-indexed elements and expects specific format.
    """
    # Extract surface faces (simplified - you may need proper surface extraction)
    # For now, we'll let redbird's meshprep handle this
    face = np.array([])  # Redbird can work without explicit faces if elem is provided
...
def build_redbird_prop_array(tissue_labels):
    """
    Build Redbird property array.
    Format: [mua, mus, g, n] for each tissue type
    Row 0 is background (unused in volume)
    """
...
def run_redbird_forward(nodes, face, elem, seg, srcpos, srcdir, detpos, detdir, prop, omega):
    ...
    Returns fluence at all nodes and at detector positions.
    """
    cfg = {
        "node": nodes,
        "face": face,
    ...
    # Prepare mesh (only needs to be done once, but we do it each time for simplicity)
    cfg, sd = rb.meshprep(cfg)
    
    # Solve forward problem
    detphi, phi = rb.run(cfg)
    
    # phi is [nodes x sources], detphi is [detectors x sources]
    return phi, detphi


def run_sensitivity_redbird(data):
    """
    Run sensitivity analysis using Redbird diffusion solver.
    Uses adjoint method: sensitivity = phi_forward * phi_adjoint
    """
    print("\n" + "=" * 70)
    ...
    print(f"  Omega: {REDBIRD_OMEGA:.2e} rad/s")
    ...
    # Create a dummy detector array for forward (just need one far away)
    ...
    for si in range(ns):
        ...
        try:
            ...
            eta = (ns - si - 1) * np.mean(fwd_times)

            phi_fwd[:, si] = phi.flatten()
            max_val = np.abs(phi_fwd[:, si]).max()
            print(f" ✓ max={max_val:.2e} ({elapsed:.1f}s | ETA {eta:.0f}s)")
    ...
    for di in range(nd):
        ...
        try:
            ...
            elapsed = time.time() - t0
            adj_times.append(elapsed)
            eta = (nd - di - 1) * np.mean(adj_times)
    ...
    _print_timing_summary(fwd_times, adj_times, time.time() - t_sim_start, ns, nd)
    ...
    for si in range(ns):
        for di in range(nd):
            # For diffusion: sensitivity = |phi_fwd| * |phi_adj|
    ...
    t_sens = time.time() - t_sens_start
    print(f"  Sensitivity computation: {t_sens:.3f}s")

    # Save
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    np.savez(os.path.join(OUTPUT_DIR, 'sensitivity_results_redbird.npz'),
             sens_map=sens_map, sens_log=sens_log,
             phi_fwd=phi_fwd, phi_adj=phi_adj,
             sources=data['sources'], detectors=data['detectors'],
             nodes=nodes, translation=data['translation'])
    print(f"  Saved: {OUTPUT_DIR}/sensitivity_results_redbird.npz")
    
    return {'sens_map': sens_map, 'sens_log': sens_log}


def run_sensitivity_mmc(data):
    """Original MMC-based sensitivity analysis."""
    print("\n" + "=" * 70)
    ...
    print("\n  Forward simulations...")
    for si in range(ns):
        if si in SKIP_SOURCES:
            print(f"    S{si+1}/{ns} SKIPPED")
            continue
        ...
        try:
            ...
            if 'flux' in res:
                ...
                print(f" ✓ max={phi_fwd[:, si].max():.2e} ({elapsed:.1f}s | ETA {eta:.0f}s)")
            else:
                print(f" ✗ no flux ({elapsed:.1f}s | ETA {eta:.0f}s)")
    ...
    print("\n  Adjoint simulations...")
    for di in range(nd):
        if di in SKIP_DETECTORS:
            print(f"    D{di+1}/{nd} SKIPPED")
            continue
        ...
        try:
            ...
            if 'flux' in res:
                ...
                print(f" ✓ max={phi_adj[:, di].max():.2e} ({elapsed:.1f}s | ETA {eta:.0f}s)")
            else:
                print(f" ✗ no flux ({elapsed:.1f}s | ETA {eta:.0f}s)")
    ...
    _print_timing_summary(fwd_times, adj_times, time.time() - t_sim_start, ns, nd, NPHOTON)
    ...
    # Save
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    np.savez(os.path.join(OUTPUT_DIR, 'sensitivity_results_mmc.npz'),
             sens_map=sens_map, sens_log=sens_log,
             phi_fwd=phi_fwd, phi_adj=phi_adj,
             sources=data['sources'], detectors=data['detectors'],
             nodes=nodes, translation=data['translation'])
    print(f"  Saved: {OUTPUT_DIR}/sensitivity_results_mmc.npz")
...
# MAIN FUNCTIONS
# =============================================================================
def load_mesh_and_register_optodes():
    print("=" * 70)
    ...
    # Load mesh
    t_load = time.time()
    mat_data = sio.loadmat(MESH_PATH)
    mesh1 = mat_data['mesh1'][0, 0]
    nodes = np.array(mesh1['node'], dtype=np.float64)[:, :3]
    elems = np.array(mesh1['elem'], dtype=np.int32)
    t_load = time.time() - t_load

    print(f"  Mesh: {len(nodes):,} nodes, {len(elems):,} elements  (loaded in {t_load:.2f}s)")
    
    # Get tissue labels
    tissue_labels = elems[:, 4].astype(np.int32) if elems.shape[1] >= 5 else np.ones(len(elems), dtype=np.int32)
    tissue_labels[tissue_labels < 0] = 0
    
    # Get reference surface and compute translation
    ref_obj = bpy.data.objects.get('Head_Surface_Overlay') or bpy.data.objects.get('headmesh')
    ...
    headmesh = bpy.data.objects.get('headmesh') or ref_obj
    ...
    print(f"\n  Registered: {len(sources)} sources, {len(detectors)} detectors  ({t_reg:.2f}s)")
    
    # Build optical properties (for MMC)
    max_label = max(tissue_labels.max(), 5)
    props = [[0, 0, 1, 1]]
    for label in range(1, max_label + 1):
        p = OPTICAL_PROPERTIES.get(label, {'mua': 0.01, 'mus': 1.0, 'g': 0.9, 'n': 1.37})
...
def visualize_on_cortex(results, data, smooth_iterations=3):
    print("\n" + "=" * 70)
    ...
    if not cortex:
        print("  ✗ No cortex object found (need 'Brain_Cortex' or 'Brain_Cortex_5L')")
        return
    ...
    all_sens_log = results['sens_log']
    all_nodes    = data['nodes']
    valid_mask   = all_sens_log > -19
    ...
    peak_log = pct_vals[pcts.index(95)]
    vmax = peak_log
    vmin = peak_log - DYNAMIC_RANGE
    print(f"  Display window: log10 [{vmin:.1f}, {vmax:.1f}]  (p95 anchor)")
    ...
    weights = 1.0 / (dists + 0.5)
    weights[dists > FILL_RADIUS] = 0.0
    ...
    print(f"  Norm: min={n_vals.min():.3f}  max={n_vals.max():.3f}  "
    ...
    for poly in mesh.polygons:
        for li in poly.loop_indices:
            ...
            if in_region[vi]:
                ...
                vc.data[li].color = (0.0, 0.0, 0.0, 0.0)  # alpha=0 → original brain material
    ...
    orig = nt.nodes.new('ShaderNodeBsdfPrincipled')
    heat = nt.nodes.new('ShaderNodeBsdfPrincipled')
    ...
    nt.links.new(vcn.outputs['Alpha'], mix.inputs['Fac'])
...
if __name__ == "__main__":
    ...
    try:
        data = load_mesh_and_register_optodes()

        # Run appropriate solver
        if SOLVER == 'redbird':
            results = run_sensitivity_redbird(data)
        else:
            results = run_sensitivity_mmc(data)

        if results:
            t_viz = time.time()
            visualize_on_cortex(results, data, smooth_iterations=SMOOTH_ITERATIONS)
            t_viz = time.time() - t_viz

            t_wall = time.time() - t_wall_start
            print("\n" + "=" * 70)
            print(f"✓ COMPLETE ({SOLVER.upper()})")
            if SOLVER == 'mmc':
                print(f"  Note: For smoother MMC results, increase NPHOTON (currently {NPHOTON})")
            print(f"  Visualization:   {t_viz:.1f}s")
            print(f"  Total wall time: {t_wall:.1f}s  ({t_wall / 60:.2f} min)")
            print("=" * 70)
        else:
            print("\n✗ FAILED")
