...
if localIsMouseOverDataAnnotatable(fig)
...
if ~docreate
    ...
    if ~isempty(hTargetBehavior) && isvalid(hTargetBehavior)
        ...
        % If the Interpreter property is set on the behavior object, use it
        % to set it on the datacursormode as well
        hTool.Interpreter = hTargetBehavior.Interpreter;
    end
end
...
%-----------------------------------------------%
function flag = localIsMouseOverDataAnnotatable(obj)
    flag = false;
    obj = ancestor(obj,'figure');    
    if ~isa(obj,'matlab.ui.Figure') || strcmp(obj.Visible, 'off')
        % this is not necessary if obj is not a figure or obj is invisible
        return;
    end

% This function determines if the mouse pointer is over a dataAnnotatable object.    
    sv = findobjinternal(obj, '-isa', 'matlab.graphics.primitive.canvas.Canvas', '-depth', 1);
    hitObj = [];
    screenPostion = get(groot,'PointerLocation');
    figPostion = getpixelposition(obj);
    currentPoint = [screenPostion(1)-figPostion(1) screenPostion(2)-figPostion(2)];
    % The hittest method assumes that the y coordinate zero point is at the top
    for i=1:length(sv)
        if ~isempty(sv(i))
            ySv = figPostion(4)- currentPoint(2);
            hitObj = sv(i).hittest(currentPoint(1),ySv);
        end
        if ~isempty(hitObj) && isa(hitObj,'matlab.graphics.chart.interaction.DataAnnotatable')
            flag = true;
        end
    end    
