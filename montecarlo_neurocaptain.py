...
def run_sensitivity_mmc(data, settings):
    ...
    gpu_id = settings.get('gpu_id', '01')
    
    print(f"  Sources: {ns}, Detectors: {nd}, Nodes: {nn:,}")
    print(f"  Photons: {nphoton:,}, GPU ID: {gpu_id}")
    ...
    # Keep background nodes at -20 (log10(1e-20)); do NOT raise them to min_valid.
    # Raising them corrupts normalization in visualize_on_cortex.
...
# =============================================================================
def load_mesh_and_register_optodes(mesh_path):
    """Load mesh from file and register optode positions from Blender."""
    print("=" * 70)
    ...
    if ref_obj is None:
        raise RuntimeError("No head surface found. Need 'Head_Surface_Overlay' or 'headmesh' object.")
...
def visualize_on_cortex(results, data, smooth_iterations=5):
    """Visualize sensitivity map on Brain_Cortex or Brain_Cortex_5L object."""
    print("\n" + "=" * 70)
    ...
    FILL_RADIUS   = 20.0   # mm — cortex vertices within this radius get interpolated color
    ...
    # Always recreate the layer to avoid stale colours from previous runs
    ...
    # ── Material: Mix Shader — alpha=0 → brain gray, alpha=1 → heatmap ───────
...
# =============================================================================
# BLENDER OPERATOR INTEGRATION FUNCTIONS
# =============================================================================
def run_forward_simulation_mmc(mesh_path, output_dir, context):
    """Main function to run MMC simulation from Blender operator."""
    nc_settings = context.scene.neurocaptain_settings
    
    if not MMC_AVAILABLE:
        raise RuntimeError("MMC (pmmc) not available. Install pmmc package.")
    
    # Get settings from UI
    settings = {
        'nphoton': nc_settings.mmc_nphoton,
        'gpu_id': nc_settings.mmc_gpu_id if nc_settings.mmc_use_gpu else None,
        'sd_max_distance': nc_settings.sd_max_distance,
    }
    
    t_wall_start = time.time()
    print("\n" + "=" * 70)
    print("MMC FORWARD SIMULATION")
    print("=" * 70 + "\n")

    try:
        # Load mesh and register optodes
        data = load_mesh_and_register_optodes(mesh_path)

        # Run MMC sensitivity analysis
        results = run_sensitivity_mmc(data, settings)

        if results:
            # Save results
            output_path = os.path.join(output_dir, 'sensitivity_mmc.npz')
            save_results(results, data, output_path, 'mmc')

            # Visualize
            t_viz = time.time()
            visualize_on_cortex(results, data, smooth_iterations=nc_settings.smooth_iterations)
            t_viz = time.time() - t_viz

            t_wall = time.time() - t_wall_start
            print("\n" + "=" * 70)
            print(f"✓ MMC COMPLETE")
            print(f"  Photons:          {settings['nphoton']:,}")
...
def run_forward_simulation_redbird(mesh_path, output_dir, context):
    """Main function to run Redbird simulation from Blender operator."""
    nc_settings = context.scene.neurocaptain_settings
    
    if not REDBIRD_AVAILABLE:
        raise RuntimeError("Redbird not available. Install with: pip install redbirdpy")
    
    # Get settings from UI
    settings = {
        'mode': nc_settings.redbird_mode,
        'frequency': nc_settings.redbird_frequency,
        'sd_max_distance': nc_settings.sd_max_distance,
    }
    
    t_wall_start = time.time()
    print("\n" + "=" * 70)
    print("REDBIRD FORWARD SIMULATION")
    print("=" * 70 + "\n")
    ...
    try:
        ...
        # Run Redbird sensitivity analysis
        results = run_sensitivity_redbird(data, settings)

        if results:
            # Save results
            output_path = os.path.join(output_dir, 'sensitivity_redbird.npz')
            save_results(results, data, output_path, 'redbird')
            ...
            t_wall = time.time() - t_wall_start
            print("\n" + "=" * 70)
            print(f"✓ REDBIRD COMPLETE")
            print(f"  Mode: {settings['mode']}")
            if settings['mode'] == 'FD':
                print(f"  Frequency: {settings['frequency']} MHz")
            print(f"  Visualization:    {t_viz:.1f}s")
            print(f"  Total wall time:  {t_wall:.1f}s  ({t_wall / 60:.2f} min)")
            print("=" * 70)
            return True
